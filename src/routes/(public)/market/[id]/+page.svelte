<script lang="ts">
    ...
    const formatKES = (v: number) => `KES ${new Intl.NumberFormat('en-KE').format(v)}`;
...
<header class="sticky top-0 z-30 border-b border-border/60 bg-background/80 backdrop-blur">
    ...
    <a href="/signup" class="ml-2 inline-flex md:hidden rounded-md bg-primary px-2 py-2 text-xs text-primary-foreground hover:opacity-90">Sign Up</a>
    {/if}
    </div>
...
<main class="mx-auto w-full max-w-[1400px] px-4 md:px-6 py-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="rounded-xl border border-border bg-card">
            <div class="p-4 border-b border-border/60 text-sm font-medium">Price chart</div>
            <div class="h-[360px] m-4 rounded border border-dashed border-border/70 bg-muted/10
        ...
        <!-- Market context -->
        <div class="rounded-xl border border-border bg-card">
            <div class="p-4 border-b border-border/60 text-sm font-medium">Market Context</div>
    ...
    <aside class="md:col-span-1">
        <div class="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <button class="rounded-md bg-neutral px-2 py-1 text-xs">+KES 100</button>
            <button class="rounded-md bg-neutral px-2 py-1 text-xs">+KES 500</button>
            <button class="rounded-md bg-neutral px-2 py-1 text-xs">+KES 1,000</button>
